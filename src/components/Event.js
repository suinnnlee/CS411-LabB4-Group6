...
const Event = ({ event, index }) => {
    ...
    return (
            ...
            <button className="card">
                ...
                <div className="thumbnail" onClick={() => openInNewTab(event.url)}>
                    <img src={event.images[9].url} alt="event_image"/>
                </div>

                <div className="CardInfo" >
                    <div className="CardText" onClick={() => openInNewTab(event.url)}>
                        <h2 className="text_shadow" title={event.name}>{event.name} </h2>
                        <p style={{color: '#959595'}}><strong>{date}</strong></p>
                    </div>
                    <div className="SaveIcon" >
                            <button className="log-button" color="blue"onClick={handleSave}>{save}</button>
                    </div>
                </div>
            </button>
    )
